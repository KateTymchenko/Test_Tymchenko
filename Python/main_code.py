...
def main():
    ...
    df.to_excel("banks_statemetns.xlsx", index=False)
    ...
    fin["outcome"] = fin.apply(
        lambda x: (
            x["commission_banks"]
            if x["operation_type"] == "income"
            else x["amount"] if x["operation_type"] == "outcome" else 0
        ),
        axis=1,
    )
    ...
    fin["outcome_operations"] = fin.apply(
        lambda x: x["amount"] if x["operation_type"] == "outcome" else 0, axis=1
    )
    ...
    fin_banks = (
        ...
        .agg(
            ...
            outcome_service_banks=("price_per_month", "max"),
    ...
    # Drop 'outcome_not_full' column
    fin_banks = fin_banks.drop(columns=["outcome_not_full"])
    ...
    try:
        ...
        with pd.ExcelWriter("financial_report.xlsx") as writer:
            turnover.to_excel(
                writer, sheet_name="Report 1", index=False, header=False, startrow=2
            )
            turnover_type.to_excel(
                writer, sheet_name="Report 2", index=False, header=False, startrow=2
            )
